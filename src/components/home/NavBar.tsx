...
const NavBar = ({ user, role }: { user: User | null, role: Role | null }) => {
    ...
    <header className='sticky top-0 z-50 flex justify-between items-center shadow-lg bg-card'>
      ...
      <div className="flex items-center">
        <div className="hidden lg:flex items-end h-full mr-8 p-4">
          <Navigation user={user} isMobile={false} role={role} />
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px]">
              ...
              <div className="mt-4 flex flex-col gap-4">
                <Navigation user={user} isMobile={true} onItemClick={() => setIsOpen(false)} role={role} />
              </div>
            </SheetContent>
          </Sheet>
        </div>
...
const Navigation = ({ user, isMobile, onItemClick, role }: { user: User | null, isMobile: boolean, onItemClick?: () => void, role: Role | null }) => (
  <div className={`flex ${isMobile ? 'flex-col' : 'gap-5'} items-center`}>
    {user ? (
      <AuthenticatedNav name={user.firstName} isMobile={isMobile} onItemClick={onItemClick} role={role} />
    ) : (
...
const AuthenticatedNav = ({ name, isMobile, onItemClick, role }: { name: string, isMobile: boolean, onItemClick?: () => void, role: Role | null }) => {
  if (!role) return null
  return (
