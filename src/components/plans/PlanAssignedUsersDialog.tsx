...
interface PlanAssignedUsersDialogProps {
  open: boolean
...
export function PlanAssignedUsersDialog({ open, onOpenChange, users, planName }: PlanAssignedUsersDialogProps) {
  return (
        ...
        <ScrollArea className="max-h-[60vh]">
          <div className="space-y-4 py-2">
            {users.length > 0 ? (
              ...
              <p className="text-center text-sm text-muted-foreground">No hay usuarios asignados a este plan</p>
            )}
          </div>
        </ScrollArea>
      </DialogContent>
