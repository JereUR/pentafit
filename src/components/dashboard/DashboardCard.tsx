  ...
  if (error) {
    return (
      ...
      >
        ...
        <div className="relative p-5 h-[130px] flex flex-col justify-between z-10">
          <div className="flex items-center gap-3">
            <div className="relative w-9 h-9">
              <div className="absolute inset-0 rounded-full bg-destructive/10" />
              <div className="absolute inset-0 flex items-center justify-center text-destructive">
                <div className="w-5 h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full">{icon}</div>
              </div>
            </div>
      ...
      <div className="relative p-5 h-[130px] flex flex-col justify-between z-10">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 flex items-center justify-center bg-background rounded-full">
            <div className={cn("w-5 h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full", iconColor)}>
              {icon}
            </div>
          </div>
          <h3 className="font-medium text-sm md:text-base">{title}</h3>
        ...
        {loading ? (
          <div className="flex justify-center items-center h-12">
            <Loader2 className={cn("animate-spin w-6 h-6", iconColor)} />
          </div>
        ) : (
          <div className="flex justify-center mt-2">
            <span className={cn("text-3xl md:text-4xl font-bold transition-all duration-500", iconColor)}>
              {value || 0}
            </span>
          </div>
        )}
